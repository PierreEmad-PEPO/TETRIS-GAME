...
public:
    ...
    void generateShape()
    {
        ...
        max_length=0,max_width=0,maxx=0,maxy=0,minx=1e9,miny=1e9;
        ...
        if(miny<0)
        {
            ...
            maxy=0,miny=1e9;
            for(int i=0;i<points.size();i++)
                points[i].y+=(0-tempy),maxy=max(maxy,points[i].y),miny=min(miny,points[i].y);
        }
    }
};
...
void Rotate(Shape& shape)
{
    ...
    for(int i=0;i<shape.points.size();i++)
    {
        ...
        if(invalid[rotated[i].x][rotated[i].y] || rotated[i].x<0 || rotated[i].x>=ScreenX || rotated[i].y<0 || rotated[i].y>=ScreenY)
            return;
    }
...
bool Update(Shape& shape)
{
    ...
    for(int i=0;i<shape.points.size();i++)
    {
        if(invalid[shape.points[i].x+1][shape.points[i].y])
        {
            setShape=1;
            break;
        }
